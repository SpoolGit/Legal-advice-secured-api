...
async def get_legal_advice(
    ...
    try:
        ...
        is_safe, banned_words_found, reason = validate_input_safety(
        ...
        response_data = await llm_service.generate_response(
            request.input_prompt, request.user_id
        )
        ...
        if "error" in response_data:
            if response_data["error"] == "response_validation_failed":
                ...
                logger.error(
                    f"LLM service error for user {request.user_id}: {response_data['error']}"
                )
                return JSONResponse(
                    status_code=500,
                    content=jsonable_encoder(
        ...
        logger.info(
            f"Legal advice generated for user {request.user_id} in {processing_time:.2f}s"
        )
