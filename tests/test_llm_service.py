...
class TestLLMService:
    ...
    async def test_safe_response_generation(self, llm_service):
        ...
        with patch('openai.OpenAI') as mock_openai_class:
            ...
            mock_response.choices[0].message.content = "This is safe legal information about contracts."
    ...
    async def test_response_validation_failure(self, llm_service):
        ...
        with patch('openai.OpenAI') as mock_openai_class:
            ...
            mock_response = MagicMock()
            mock_response.choices = [MagicMock()]
            mock_response.choices[0].message.content = "You should kill your legal opponent."
            mock_response.usage = None
            
            mock_client.chat.completions.create.return_value = mock_response
            
            # Mock validator ot detect banned words
            with patch('app.utils.validator.contains_banned_words', return_value=['kill']):
                result = await llm_service.generate_response(
                    "How to win a case?", 
                    "test_user"
                )
    ...
    async def test_response_validation_success(self, llm_service):
        ...
        with patch('openai.OpenAI') as mock_openai_class:
            ...
            mock_response = MagicMock()
            mock_response.choices = [MagicMock()]
            mock_response.choices[0].message.content = "Legal contracts require offer, acceptance, and consideration."
